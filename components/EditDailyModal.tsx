...
interface EditDailyModalProps {
  ...
  onSave: (id: number, updatedDaily: any) => void
}

export default function EditDailyModal({ isVisible, onClose, daily, onSave }: EditDailyModalProps) {
  ...
  // Update form when daily changes
  useEffect(() => {
    if (daily) {
      setTitle(daily.title)
      setDescription(daily.description || "")
      setPriority(daily.priority || 'medium')
      setDifficulty(daily.difficulty || 'medium')
      setCategory(daily.category || "")
    }
  }, [daily])
